        ...
        <main class="main-content">
            <header class="catalog-header"
                ...
                <button class="btn-add" onclick="openModal()"
                    style="background: #1e3a8a; color: white; padding: 12px 20px; border: none; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 8px;">
            ...
            <div class="toolbar" style="display: flex; gap: 20px; margin-bottom: 25px;">
                <div class="search-bar" style="flex: 1; position: relative;">
                    <input type="text" id="catalogSearch" placeholder="Search title or author..."
                        style="width: 100%; padding: 12px 40px; border: 1px solid #e2e8f0; border-radius: 8px;">
                    <i class="fi fi-rr-search" style="position: absolute; left: 15px; top: 14px; color: #94a3b8;"></i>
                </div>
                <div class="filter-group" style="display: flex; gap: 10px;">
                    <select class="select-filter" id="genreFilter"
            ...
            <div class="book-grid" id="bookGrid">
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()):
                        ...
                        ?>
                        <div class="book-card" data-genre="<?php echo htmlspecialchars($row['Genre_genre_id']); ?>"
                            onclick="window.location.href='book_details.php?id=<?php echo $row['book_id']; ?>'"
                            style="background: white; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); overflow: hidden; cursor: pointer;">
                            ...
                            <div class="book-details" style="padding: 15px;">
                                <span
                                    style="font-size: 0.7rem; color: #3b82f6; font-weight: 700; text-transform: uppercase;"><?php echo htmlspecialchars($row['Genre_genre_id']); ?></span>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <p style="text-align: center; color: #94a3b8; padding: 40px;">No books found in the catalog.</p>
                <?php endif; ?>
            </div>
        </main>
            ...
            <div class="modal-container">
                <header class="form-header"
                    ...
                    <div>
                        ...
                        <p style="margin:0; font-size: 0.8rem; opacity: 0.8;">Fill in metadata to update your
                            collection.</p>
                    </div>
                ...
                <form id="bookForm" style="padding: 25px;">
                    <div style="display: grid; grid-template-columns: 200px 1fr; gap: 30px;">
                        ...
                        <div class="col-right">
                            <div style="margin-bottom: 15px;">
                                ...
                                <div id="apiResults"
                                    style="display:flex; gap:10px; overflow-x:auto; margin-top:10px; padding: 5px 0;">
                                </div>
                            </div>
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                ...
                                <div>
                                    <label style="display:block; font-weight:600;">Edition</label>
                                    <input type="text" id="modalEdition"
                                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px;">
                                </div>
                                <div>
                                    <label style="display:block; font-weight:600;">ISBN</label>
                                    <input type="text" id="modalISBN"
                                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px;">
                                </div>
                                <div>
                                    <label style="display:block; font-weight:600;">Publisher</label>
                                    <input type="text" id="modalPublisher"
                                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px;">
                                </div>
                                <div>
                                    <label style="display:block; font-weight:600;">Publish Date</label>
                                    <input type="text" id="modalYear" placeholder="YYYY-MM-DD"
                                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px;">
                                </div>
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top:15px;">
                                <div>
                                    ...
                                    <input type="number" id="modalCopies" value="1"
                                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px;">
                                </div>
                                <div>
                                    <label style="display:block; font-weight:600;">Price (₱)</label>
                                    <input type="number" id="modalPrice" value="0"
                                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px;">
                                </div>
                                ...
                                <div class="form-group">
                                    <label>Genre</label>
                                    <select name="genre_id" id="bookGenre"
                                        style="width: 100%; padding: 10px; border: 1px solid #e2e8f0; border-radius: 8px;">
                                        <option value="">Select Genre</option>
                                        <?php
                                        if ($genres_result && $genres_result->num_rows > 0) {
                                            while ($genre = $genres_result->fetch_assoc()) {
                                                // Use genre_id for the value (to save to DB) and genre_name for the text
                    ...
                    <div class="form-actions"
                        ...
                        <button type="button" onclick="closeModal()"
                            style="padding: 10px 20px; margin-right: 10px; background:white; border:1px solid #cbd5e1; border-radius:6px; cursor:pointer;">Cancel</button>
                        <button type="submit"
                            style="padding: 10px 25px; background:#1e3a8a; color:white; border:none; border-radius:6px; font-weight:600; cursor:pointer;">Publish
                            to Catalog</button>
                    </div>
                </form>
            </div>
                    ...
                    data.items.slice(0, 4).forEach(book => {
                        ...
                        thumb.onclick = () => {
                            ...
                            if (info.industryIdentifiers) {
                                const id = info.industryIdentifiers.find(i => i.type === 'ISBN_13');
                                isbn13 = id ? id.identifier : info.industryIdentifiers[0].identifier;
                            }
                            ...
                            document.getElementById('modalISBN').value = isbn13;
